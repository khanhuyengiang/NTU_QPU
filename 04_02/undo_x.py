...
Q1 = 5 # +1 for the actual qubit name
Q2 = 6
drive_qubit = Q2
...
pi_pulse_duration = pi_pulse.finish
idle_duration = 6*pi_pulse_duration
print(f'Pi pulse duration: {pi_pulse_duration}')

# Save pulse times and run parameters
pulse_times = np.linspace(0, pi_pulse_duration, 15)
idle_times = np.linspace(pulse_times[-1], pulse_times[-1] + idle_duration, 15)
undo_times = np.linspace(idle_times[-1], idle_times[-1] + pi_pulse_duration, 15)


times = np.append(pulse_times, idle_times)
times = np.append(times, undo_times)
amplitude = 0.2
np.save('x_pulse_times.npy', times)
np.save('run_params.npy', [pi_pulse_duration, Q1, Q2, drive_qubit, amplitude])
...
for t in times:
    ...
    # Add readout pulse and execute for Q1 and Q2
...
for t in idle_times:
    ps = PulseSequence()
    drive_pulse = platform.create_qubit_drive_pulse(qubit=drive_qubit, start=0, duration=pi_pulse_duration)
...
for t in undo_times:
    ...
    # Create initial X pulse and undo X pulse
    drive_pulse = platform.create_qubit_drive_pulse(qubit=drive_qubit, start=0, duration=pi_pulse_duration)
    undo_pulse = platform.create_qubit_drive_pulse(qubit=drive_qubit, start=idle_times[-1], duration=t)
